...
import { SimulationEngine } from './simulationEngineAE.ts';
import type { UserConfig, Mode, EngineStepStats } from './typesAE.ts';
import * as StatsNS from './statsAE.ts';

function printHelpAndExit() {
  const msg = `
Usage:
  npm run expAE -- [options]

Required:
  --outDir <path>          Output directory (e.g., results/fig1)

Core options:
  --seedBase <int>         Base RNG seed (e.g., 1000)
  --seeds <int>            Number of paired seeds (e.g., 50)
  --maxAgents <int>        Maximum number of agents (e.g., 1000)

Environment:
  --inflowStart <float>   Inflow scan start
  --inflowEnd <float>     Inflow scan end
  --inflowStep <float>    Inflow scan step

Noise:
  --noise <float>         Single noise level (default)
  --noiseList <list>      Comma-separated noise list (overrides --noise)
                          e.g., --noiseList 0.0,0.02,0.04

Cost scan (for Fig.8):
  --costStart_i <int>     Cost index start (implementation-specific)
  --costEnd_i <int>       Cost index end
  --costStep_i <int>      Cost index step

Notes:
  - Paired design: informed and random runs share the same seeds.
  - If both --noise and --noiseList are provided, --noiseList takes precedence.
  - Results will be written to <outDir>/{raw,agg,survival}.

Examples:
  npm run expAE -- --outDir results/fig1 --seedBase 1000 --seeds 50 --maxAgents 1000 \\
    --noise 0 --inflowStart 0 --inflowEnd 0.172 --inflowStep 0.002

  npm run expAE -- --outDir results/fig9 --seedBase 1000 --seeds 50 --maxAgents 1000 \\
    --noiseList 0.0,0.02,0.04,0.06,0.08,0.1 --inflowStart 0 --inflowEnd 0.2 --inflowStep 0.005
`;
  console.log(msg);
  process.exit(0);
}
...
function assertFiniteStats(s: EngineStepStats, ctx: string) {
  const bad =
    ...
    !Number.isFinite(s.agentExpenditure);
  ...
  if (bad) {
    throw new Error(
      ...
      `uptake=${s.uptake} info=${s.infoCost} maint=${s.maintenanceCost} div=${s.divisionCost} expend=${s.agentExpenditure}`
    );
  }
...
type RunSummary = {
  ...
  // NEW: alternative proxy (cumulative)
  agentExpenditureCum: number;
  ...
  // NEW: alternative proxy (window mean)
  mean_agentExpenditure_window: number;
...
const sweep: Sweep = {
  inflowStart: 0.0, inflowEnd: 0.3, inflowStep: 0.005,
...
function runOne(
  ...
  // NEW: window sum for alternative proxy
  let sumAgentExpendW = 0;
  ...
  // NEW: cumulative alternative proxy
  let agentExpenditureCum = 0;
  ...
  function pushTail(uptakeTick: number, infoTick: number, maintTick: number, divTick: number, birthsTick: number, deathsTick: number) {
  ...
  while (t < maxTicks) {
    ...
    // NEW: cumulative alternative proxy
    agentExpenditureCum += s.agentExpenditure;
    ...
    pushTail(s.uptake, s.infoCost, s.maintenanceCost, s.divisionCost, birthsTick, deathsTick);
    ...
    if (t >= burnIn && t < burnIn + measureTicks) {
      ...
      // NEW: window sum
      sumAgentExpendW += s.agentExpenditure;
    ...
    if (s.agentCount === 0) {
      ...
      return {
        ...
        mean_agentExpenditure_window: measuredTicksDone ? (sumAgentExpendW / measuredTicksDone) : 0,
...
function makeRunRecordNoId(
  ...
  return {
    ...
    agentExpenditureCum: sum.agentExpenditureCum,
    ...
    mean_agentExpenditure_window: sum.mean_agentExpenditure_window,
...
async function main() {
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        note: 'A-mode: thermodynamic "entropy" vocabulary removed; dissipation proxies reported. Added agentExpenditure proxy.',
        effectiveParams: {
  ...
  writeFileSync(
    ...
    toCSV(
      [
        ...
        'tailTicksUsed', 'mean_uptake_tail', 'mean_infoCost_tail', 'mean_maintenance_tail', 'mean_divisionCost_tail', 'mean_efficiency_tail',
      ...
      raw.map(r => [
        ...
        // NEW
        r.agentExpenditureCum.toFixed(6),
        ...
        // NEW
        r.mean_agentExpenditure_window.toFixed(6),
  ...
  for (const g of groups.values()) {
    ...
    // NEW: paired difference for alternative proxy
    const dAgentExpendCum: number[] = [];
    ...
    for (const pair of bySeed.values()) {
      ...
      // NEW
      dAgentExpendCum.push(pair.inf.agentExpenditureCum - pair.rand.agentExpenditureCum);
    ...
    // NEW
    const ciAgentExpendCum = bootstrapCI(dAgentExpendCum, mean, 2000);
    ...
    logrankOut.push({
      ...
      dMean_sigmaProxyDiffCumEnd: mean(dSigmaProxyDiffCum),
      ciLo_dSigmaProxyDiffCumEnd: ciSigmaProxyDiffCum.lo,
      ciHi_dSigmaProxyDiffCumEnd: ciSigmaProxyDiffCum.hi,

      dMean_sigmaProxyActCumEnd: mean(dSigmaProxyActCum),
      ciLo_dSigmaProxyActCumEnd: ciSigmaProxyActCum.lo,
      ciHi_dSigmaProxyActCumEnd: ciSigmaProxyActCum.hi,

      // NEW
      dMean_agentExpenditureCum: mean(dAgentExpendCum),
      ciLo_dAgentExpenditureCum: ciAgentExpendCum.lo,
      ciHi_dAgentExpenditureCum: ciAgentExpendCum.hi,

      dMean_outflowNumCum: mean(dOutflowNumCum),
      ciLo_dOutflowNumCum: ciOutflowNumCum.lo,
      ciHi_dOutflowNumCum: ciOutflowNumCum.hi,
      ...
      // supplementary (independence-assuming) logrank
      logrank_z: lr.z,
    ...
    aggRows.push([
      ...
      mean(dSigmaProxyDiffCum).toFixed(6),
      sd(dSigmaProxyDiffCum).toFixed(6),
      ciSigmaProxyDiffCum.lo.toFixed(6),
      ciSigmaProxyDiffCum.hi.toFixed(6),

      mean(dSigmaProxyActCum).toFixed(6),
      sd(dSigmaProxyActCum).toFixed(6),
      ciSigmaProxyActCum.lo.toFixed(6),
      ciSigmaProxyActCum.hi.toFixed(6),

      // NEW
      mean(dAgentExpendCum).toFixed(6),
      sd(dAgentExpendCum).toFixed(6),
      ciAgentExpendCum.lo.toFixed(6),
      ciAgentExpendCum.hi.toFixed(6),

      mean(dOutflowNumCum).toFixed(6),
      sd(dOutflowNumCum).toFixed(6),
      ciOutflowNumCum.lo.toFixed(6),
      ciOutflowNumCum.hi.toFixed(6),
  ...
  writeFileSync(
    ...
    toCSV(
      [
        ...
        'dMean_sigmaProxyDiffCumEnd', 'sd_dSigmaProxyDiffCum', 'ciLo_dSigmaProxyDiffCumEnd', 'ciHi_dSigmaProxyDiffCumEnd',
        'dMean_sigmaProxyActCumEnd', 'sd_dSigmaProxyActCum', 'ciLo_dSigmaProxyActCumEnd', 'ciHi_dSigmaProxyActCumEnd',

        // NEW
        'dMean_agentExpenditureCum', 'sd_dAgentExpendCum', 'ciLo_dAgentExpendCum', 'ciHi_dAgentExpendCum',

        'dMean_outflowNumCum', 'sd_dOutflowNumCum', 'ciLo_dOutflowNumCum', 'ciHi_dOutflowNumCum',
