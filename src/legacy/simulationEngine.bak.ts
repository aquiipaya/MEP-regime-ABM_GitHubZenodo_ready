...
export class SimulationEngine {
  ...
  private entropyFlowCum = 0;
  private entropyProdCum = 0;
  private entropyChangeCum = 0;

  private hasSpare = false;
  private spare = 0;

  // --- NEW: per-cell last-step maps for visualization ---
  private lastSigmaDiffMap: Float32Array | null = null;  // sigmaDiff per cell (already /T0)
  private lastActHeatMap: Float32Array | null = null;    // act heat per cell (NOT /T0)
  private lastSigmaActMap: Float32Array | null = null;   // sigmaAct per cell (already /T0)
  private lastSigmaTotalMap: Float32Array | null = null; // sigmaDiff+sigmaAct per cell
  ...
  public importState(state: { grid: Float32Array; cells: Cell[] }) {
    ...
    this.entropyFlowCum = 0;
    this.entropyProdCum = 0;
    this.entropyChangeCum = 0;
  ...
  // --- NEW: getters for snapshot export ---
  public getSnapshot() {
  ...
  public update(config: UserConfig & { mode: Mode }): EngineStepStats {
    ...
    const { sigmaDiff, outflowPhysical, outflowNumerical, sigmaDiffMap } = this.diffuseOneStep(config, dt);
    ...
    const sigmaAct = outflowAct / T0;
    const sigmaOut = 0;
    const sigmaTotal = sigmaDiff + sigmaAct;

    const entropyFlow = (inflow - outflowPhysical) / T0;

    const entropyProd = sigmaTotal;
    const entropyChange = entropyFlow + entropyProd;

    this.entropyFlowCum += entropyFlow;
    this.entropyProdCum += entropyProd;
    this.entropyChangeCum += entropyChange;
    ...
    if (
      ...
      !Number.isFinite(sigmaDiff) ||
      !Number.isFinite(sigmaAct) ||
      !Number.isFinite(sigmaTotal) ||
      !Number.isFinite(entropyFlow) ||
      !Number.isFinite(entropyProd) ||
      !Number.isFinite(entropyChange) ||
      !Number.isFinite(this.entropyProdCum) ||
      !Number.isFinite(this.entropyFlowCum) ||
      !Number.isFinite(this.entropyChangeCum) ||
      ...
      throw new Error(
        ...
        `sigmaDiff=${sigmaDiff} sigmaAct=${sigmaAct} sigmaTotal=${sigmaTotal} ` +
        `SeFlow=${entropyFlow} uptake=${uptake} info=${infoCost} maint=${maintenanceCost} eff=${efficiency}`
      );
    }
    ...
    const stats: EngineStepStats = {
      ...
      entropyFlowCumulative: this.entropyFlowCum,
      entropyProdCumulative: this.entropyProdCum,
      entropyChangeCumulative: this.entropyChangeCum,

      sigmaDiff,
      sigmaAct,
      sigmaOut,
      sigmaTotal,
      sigmaCumulative: this.entropyProdCum,
      ...
      externalEntropy: this.entropyFlowCum,
      internalEntropy: this.entropyProdCum,
      totalEntropy: this.entropyChangeCum,
    };
  ...
  private diffuseOneStep(config: UserConfig, dt: number) {
    ...
    // NEW: per-cell sigmaDiff map (already /T0 units)
    ...
    if (config.boundary === 'isothermal') {
      ...
      // open boundary: keep as-is (no extra sink)
    } else if (config.boundary === 'reflect') {
      // reflect: keep as-is (no extra sink)
    }

    this.grid = next;
    return { sigmaDiff, outflowPhysical, outflowNumerical, sigmaDiffMap };
  }

  private updateCells(config: UserConfig & { mode: Mode }, dt: number, T0: number, actHeatMap: Float32Array) {
    const N = config.gridSize;
    ...
    for (const c of this.cells) {
      ...
      // record maintenance heat at current cell
      ...
      if (config.mode === 'informed') {
        ...
        // record info cost heat at current cell
      ...
      if (c.energy >= divisionThreshold && nextCells.length + 2 <= maxAgents) {
        ...
        if (eAfter <= 0) {
          ...
          outflowAct += paid;
          // record what was actually paid as heat
