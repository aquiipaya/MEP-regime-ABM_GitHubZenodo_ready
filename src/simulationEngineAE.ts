// src/simulationEngine.ts AEversion
import type { UserConfig, Mode, EngineStepStats } from './typesAE';
...
export class SimulationEngine {
  ...
  public update(config: UserConfig & { mode: Mode }): EngineStepStats {
    ...
    // agent step => outflowAct and its decomposition
    let outflowAct = 0;
    let uptake = 0;
    let infoCost = 0;
    ...
    // NEW: alternative proxy (no /T0, no diffusion)
    const agentExpenditure = maintenanceCost + infoCost + divisionCost;
    ...
    if (
      ...
      !Number.isFinite(agentExpenditure) ||
      ...
      throw new Error(
        ...
        `uptake=${uptake} info=${infoCost} maint=${maintenanceCost} div=${divisionCost} expend=${agentExpenditure} eff=${efficiency}`
      );
    }
